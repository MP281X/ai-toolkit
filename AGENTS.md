...
- **Duplicate code over creating premature abstractions** - Prefer duplicated code over shared helpers. Do not create helper functions unless the exact same logic is used in 3+ places AND the abstraction clearly reduces complexity.
- **Inline the code as much as possible** - Keep logic inline at the call site. Inline unless physically impossible.
- **Colocate the code** - Code should live as close as possible to where it is used. Do not create separate files for small utilities.
- **Prefer fewer files** - Prefer larger local modules over fragmented folder structures. Delete thin wrapper files and pass-through modules.
