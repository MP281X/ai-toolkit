...
function RadioGroup(input: RadioGroupPrimitive.RadioGroup.Props) {
	const props = EffectRecord.remove('className')(input)
	return (
		<RadioGroupPrimitive.RadioGroup
			data-slot="radio-group"
			className={cn('grid w-full gap-2', input.className)}
			{...props}
		/>
	)
}
function RadioGroupItem(input: RadioPrimitive.Radio.Root.Props) {
	const props = EffectRecord.remove('className')(input)
	return (
