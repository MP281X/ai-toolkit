import {Array, HashSet, Match, Number, Option, Order, Predicate, Record, String, pipe} from 'effect'
...
class TokenNode extends Lexical.TextNode {
	...
	public static override clone(node: TokenNode) {
		return new TokenNode({id: node.__id, key: node.__key, kind: node.__kind, text: node.__text})
	}
	public static override importJSON(
		...
		return new TokenNode({id: node.id, kind: node.kind, text: node.text}).updateFromJSON(node)
	}
	...
	public constructor(input: {
		readonly id: string
		readonly key?: Lexical.NodeKey
		readonly kind: 'entry' | 'file'
		readonly text: string
	}) {
		super(input.text, input.key)
		this.__id = input.id
...
class Item<TValue extends RichTextArea.Value> extends MenuOption {
	...
	public constructor(input: {readonly entry: RichTextArea.Entry<TValue>; readonly key: string}) {
		super(input.key)
		this.entry = input.entry
	}
}
class TokenRegistry<TValue extends RichTextArea.Value> extends Map<string, RichTextArea.Token<TValue>> {}
function editorSnapshot<TValue extends RichTextArea.Value>(input: {
	readonly editor: Lexical.LexicalEditor | undefined
	...
	const editorState = input.editor.getEditorState().toJSON()
	const text = input.editor.getEditorState().read(() => {
		...
		for (const node of Lexical.$getRoot().getAllTextNodes()) {
			...
			ids.push(node.__id)
			const value = input.tokensMap.get(node.__id)
			if (value) tokens.push(value)
		}
		return {ids: HashSet.fromIterable(ids), text: String.trim(Lexical.$getRoot().getTextContent()), tokens}
	})
	for (const id of input.tokensMap.keys()) {
		if (!HashSet.has(text.ids, id)) input.tokensMap.delete(id)
	}
	return {editorState, text: text.text, tokens: text.tokens}
}
function restore<TValue extends RichTextArea.Value>(input: {
	readonly editor: Lexical.LexicalEditor | undefined
	readonly snapshot: Readonly<RichTextArea.Snapshot<TValue>>
	readonly tokensMap: Map<string, RichTextArea.Token<TValue>>
}) {
	if (!input.editor) return
	input.tokensMap.clear()
	for (const token of input.snapshot.tokens) input.tokensMap.set(token.id, token)
	input.editor.setEditorState(input.editor.parseEditorState(input.snapshot.editorState))
}
function getItems<TValue extends RichTextArea.Value>(input: {
	readonly search: {readonly trigger: string; readonly query: string} | undefined
	readonly options: Record<string, {readonly color: string; readonly values: readonly TValue[]}> | undefined
}) {
	if (!input.search) return Array.empty<Item<TValue>>()
	if (!input.options?.[input.search.trigger]) return Array.empty<Item<TValue>>()
	const noMatchScore = -1_000_000
	return pipe(
		input.options[input.search.trigger].values,
		Array.map(value => {
			const query = String.toLowerCase(input.search.query)
		...
		Array.map(
			(value, index) =>
				new Item({
					entry: {color: input.options[input.search.trigger].color, trigger: input.search.trigger, value: value.value},
					key: `${input.search.trigger}:${value.value.label}:${index}`
				})
		)
	)
}
function match(input: {readonly text: string; readonly triggers: readonly string[]}) {
	for (const trigger of input.triggers) {
		const index = input.text.lastIndexOf(trigger)
		if (index === -1) continue
		if (index > 0 && input.text[index - 1] !== '(' && !/\s/u.test(input.text[index - 1] ?? '')) continue
		const query = String.slice(index + String.length(trigger))(input.text)
		if (String.length(query) > 32 || /\s/u.test(query)) continue
		return {leadOffset: index, query, replaceableString: String.slice(index)(input.text), trigger}
	}
	return null
}
...
function lineBeforeCursor(input: {readonly text: string; readonly offset: number}) {
	const before = String.slice(0, input.offset)(input.text)
...
function continueList(event: KeyboardEvent | undefined) {
	...
	const currentLine = lineBeforeCursor({offset: current.selection.anchor.offset, text: current.node.getTextContent()})
		...
		() =>
			editor.registerCommand(
				...
				event => {
					...
					event?.preventDefault()
					props.onSubmit(editorSnapshot({editor, tokensMap: props.tokensMap}))
					return true
				},
				...
				event => {
					...
					editor.update(() => {
						...
						const selection = pipe(
							Match.value(Lexical.$isRangeSelection(currentSelection)),
							Match.when(true, () => currentSelection),
							Match.orElse(() => pipe(Lexical.$getRoot().selectEnd(), () => Lexical.$getSelection()))
						)
						...
						for (const file of files) {
							...
							selection.insertNodes([
								Lexical.$applyNodeReplacement(new TokenNode({id, kind: 'file', text: file.name}))
									.setMode('token')
		...
		<LexicalTypeaheadMenuPlugin<Item<TValue>>
			...
			onSelectOption={(option, node, close) => {
				...
				const token = Lexical.$applyNodeReplacement(
					new TokenNode({id, kind: 'entry', text: `${option.entry.trigger}${option.entry.value.label}`})
				)
					.setMode('token')
			...
			options={getItems({options: props.options, search})}
...
export declare namespace RichTextArea {
	...
	export type Props<TValue extends Value = Value> = {
		readonly handleRef?: React.Ref<Handle<TValue>>
		readonly options?: Record<string, {readonly color: string; readonly values: readonly TValue[]}>
...
export function RichTextArea<TValue extends RichTextArea.Value = RichTextArea.Value>(
	parameters: RichTextArea.Props<TValue>
) {
	...
	useImperativeHandle(
		...
		() => ({
			...
			getSnapshot() {
				return editorSnapshot({editor: editorRef.current ?? undefined, tokensMap})
			},
			restore(nextSnapshot: Readonly<RichTextArea.Snapshot<TValue>>) {
				restore({editor: editorRef.current ?? undefined, snapshot: nextSnapshot, tokensMap})
			}
		}),
		[tokensMap]
	)
					...
					setEditor={editor => {
						...
						if (editor && !initialSnapshotRestoredRef.current) {
							...
							if (Predicate.isNotUndefined(input.initialSnapshot)) {
								restore({editor, snapshot: input.initialSnapshot, tokensMap})
							}
						}
					}}
					tokensMap={tokensMap}
