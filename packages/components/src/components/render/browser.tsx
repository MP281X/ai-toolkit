...
import {Loading} from '#components/fallbacks.tsx'
...
const BrowserMessage = Schema.Union([
	...
	Schema.Struct({deslopBrowserLocation: Schema.Literal(true), path: Schema.String}),
...
export function Browser(props: {readonly className?: string; readonly origin: string}) {
	...
	const [currentUrl, setCurrentUrl] = useState(() => browserUrl(props.origin, '/'))
	...
	useEffect(() => {
		setAddress('/')
		setCurrentUrl(browserUrl(props.origin, '/'))
