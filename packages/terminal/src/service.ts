...
export class Terminal extends Context.Service<Terminal>()('@deslop/terminal/service/Terminal', {
	make: Effect.fnUntraced(function* (config: {readonly command?: ChildProcess.StandardCommand; readonly cwd: string}) {
		const callbackQueue = yield* Queue.unbounded<Effect.Effect<void>>()
		...
		const nextSequence = Ref.modify(sequenceRef, current => [current, current + 1] as const)
		...
		function setTitle(input: {
			...
			return pipe(
				...
				Effect.flatMap(current => {
					...
					if (Predicate.isUndefined(input.title)) {
						return Predicate.isUndefined(input.state)
							? Effect.void
							: publishStatus(
									input.state === 'running'
					...
					const titleStatus = terminalTitleStatus(input.title)
					return publishStatus(
						Predicate.isUndefined(input.state)
							? {...current, ...titleStatus}
							: {...current, ...titleStatus, state: input.state}
					)
				})
			)
		}
		...
		const processOutput = Effect.fnUntraced(function* (output: {readonly data: string; readonly process: IPty}) {
			...
			for (const chunk of terminalChunks({data: output.data})) {
		...
		const writeOutput = Effect.fnUntraced(function* (chunk: string) {
			const updates = yield* Ref.modify(oscRef, carry => {
				const parsed = terminalOscUpdates({carry, data: chunk})
				return [parsed.updates, parsed.carry] as const
			})
			for (const update of updates) {
				yield* update.type === 'title'
					? setTitle({state: update.state, title: update.title})
					: setProgress(update.state)
			}
			yield* Effect.promise(() => screen.write(chunk))
			yield* publishFrame({data: chunk, sequence: yield* nextSequence, type: 'output'})
		})
		const resetScreen = Effect.gen(function* () {
			yield* Effect.sync(() => {
				screen.reset()
			})
		...
		const waitForDataQueueDrain = Effect.gen(function* () {
		...
		const spawnProcess = Effect.gen(function* () {
			yield* stopProcess()
			...
			const drainBackpressure = Effect.gen(function* () {
			...
			const data = subprocess.onData(chunk => {
				...
				if (backpressureState.pending.length >= 1024) {
					Queue.offerUnsafe(
						callbackQueue,
						pipe(stopProcess('failed'), Semaphore.withPermit(lifecycleLock), Effect.ignore)
					)
					return
				}
		...
		const startDefaultProcess = Effect.gen(function* () {
		...
		return {
			attach: (size?: TerminalSize) =>
				pipe(
					...
					Stream.withSpan('Terminal.attach')
				),
			...
			restart: pipe(
				...
				Effect.withSpan('Terminal.restart')
			),
			status,
			stop: pipe(
				Effect.gen(function* () {
