...
function command(input: {readonly cwd: string; readonly taskId: string}) {
	return ChildProcess.make('vp', ['run', input.taskId], {cwd: input.cwd})
}
	...
	make: Effect.fnUntraced(function* (input: {readonly cwd: string}) {
		...
		const scripts = pipe(
			...
			Array.reduce(HashMap.empty<string, ChildProcess.StandardCommand>(), (current, scriptName) =>
				HashMap.set(current, scriptName, command({cwd: input.cwd, taskId: scriptName}))
			)
		)
		const dev = pipe(
			packageJson.deslop?.dev ?? Array.empty<string>(),
			Array.reduce(HashMap.empty<string, ChildProcess.StandardCommand>(), (current, taskId) =>
				HashMap.set(current, taskKey(taskId), command({cwd: input.cwd, taskId}))
			)
		)
		return {dev, scripts}
	})
