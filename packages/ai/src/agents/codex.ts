...
const AgentMessageDeltaNotification = Schema.Struct({
	delta: Schema.String,
...
const ItemCompletedNotification = Schema.Struct({
	...
	item: Schema.Union([
		...
		Schema.Struct({
			...
			id: Schema.String,
			changes: Schema.Array(Schema.Defect),
		...
		Schema.Struct({
			...
			tool: Schema.String,
			result: Schema.NullOr(Schema.Defect),
			error: Schema.NullOr(Schema.Defect)
		}),
		Schema.Struct({
...
const ThreadOpenResponse = Schema.Struct({thread: Schema.Struct({id: Schema.String})})
const TurnStartResponse = Schema.Struct({turn: Schema.Struct({id: Schema.String})})
const TurnCompletedNotification = Schema.Struct({threadId: Schema.String})
...
const makeCodexClient = Effect.fnUntraced(function* (config: {readonly cwd: string}) {
	...
	const request = Effect.fnUntraced(function* (method: string, params?: unknown) {
		...
		yield* pipe(writeJson(child, {id, method, ...(params !== undefined ? {params} : {})}), Effect.orDie)
...
export const makeLayerCodex = Effect.fnUntraced(function* (config: {
	...
	const thread = Schema.decodeUnknownSync(ThreadOpenResponse)(
		yield* config.sessionId
			? client.request('thread/resume', {
					threadId: config.sessionId,
					...makeThreadStartParams({cwd: config.cwd, model: 'gpt-5.1-codex', systemPrompt: config.systemPrompt})
				})
			: client.request(
					'thread/start',
					makeThreadStartParams({cwd: config.cwd, model: 'gpt-5.1-codex', systemPrompt: config.systemPrompt})
				...
				Effect.fnUntraced(function* (queue) {
					...
					yield* pipe(
						Effect.gen(function* () {
							...
							const turn = Schema.decodeUnknownSync(TurnStartResponse)(
							...
							while (true) {
								...
								if (notification.method === 'turn/started') {
									const params = Schema.decodeUnknownOption(TurnStartedNotification)(notification.params)
								...
								if (notification.method === 'item/agentMessage/delta') {
									const params = Schema.decodeUnknownOption(AgentMessageDeltaNotification)(notification.params)
									if (Option.isSome(params) && params.value.threadId === thread.thread.id) {
										yield* Queue.offer(
											queue,
								...
								if (notification.method === 'error') {
									const params = Schema.decodeUnknownOption(ErrorNotification)(notification.params)
								...
								if (notification.method === 'item/completed') {
									const params = Schema.decodeUnknownOption(ItemCompletedNotification)(notification.params)
									if (Option.isSome(params) && params.value.threadId === thread.thread.id) {
								...
								if (notification.method === 'turn/completed') {
									const params = Schema.decodeUnknownOption(TurnCompletedNotification)(notification.params)
									if (Option.isSome(params) && params.value.threadId === thread.thread.id) {
										completed = true
										yield* Ref.set(currentTurnId, Option.none())
