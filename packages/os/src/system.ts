...
export const cpuTimes = Effect.gen(function* () {
...
export function nodeProcessUsage(input?: {readonly heapLimitBytes: number; readonly heapUsedBytes: number}) {
	...
	const heapUtilization = pipe(
		Match.value(heapLimitBytes <= 0),
		Match.when(true, () => 0),
		Match.orElse(() => Math.max(0, Math.min(100, (heapUsedBytes / heapLimitBytes) * 100)))
	)
