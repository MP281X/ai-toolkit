import {Effect, Layer, pipe} from 'effect'

import {Agent} from '@ai-toolkit/ai/service'

import {AiContracts} from '#rpcs/ai/contracts.ts'

export const AiLive = pipe(
	AiContracts.toLayer(
		Effect.gen(function* () {
			const agent = yield* Agent

			return AiContracts.of({
				'ai.listMessages': () => agent.history,
				'ai.sendMessage': input => agent.prompt(input),
				'ai.tool': response => agent.respond(response)
			})
		})
	),
	Layer.provide(Agent.layer({agent: 'opencode', provider: 'github-copilot', model: 'gpt-5-mini'}))
	// Layer.provide(Agent.layer({agent: 'copilot', provider: 'github-copilot', model: 'gpt-5-mini'}))
	// Layer.provide(Agent.layer({agent: 'ai', provider: 'openrouter', model: 'openai/gpt-oss-20b:free'}))
)
