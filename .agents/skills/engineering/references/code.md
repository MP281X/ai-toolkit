...
- Remove obsolete work first; reuse a direct existing path second; compose locally third; introduce an abstraction only when it owns policy, lifecycle, a boundary, recursion, or behavior reused or changed as one unit.
...
**Reject:** IIFEs used only for expression shape, needless named intermediates, origin-hiding destructuring, signature laundering, generic patch objects, deeply nested conditionals, and compatibility wrappers.
