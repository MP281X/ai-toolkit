...
- Client and server assembly → `RpcClient.ts`, `RpcServer.ts`
- Testing → `RpcTest.ts`, `RpcTest.makeClient`
- Trust RPC schemas as the external boundary. Start with `RpcTest.ts` when the end-to-end shape is unclear.
