...
- Derive the pull-request title and a new body from the complete current branch diff against its target without reading or accumulating the old body. The body contains exactly `Summary` and `Changes`. Do not add validation, process, tool, or agent sections.
